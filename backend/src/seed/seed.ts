...
async function seed() {
  ...
  const requestModel = app.get<Model<MaintenanceRequest>>(getModelToken(MaintenanceRequest.name));
  ...
  const locations = await locationModel.insertMany([
    { name: 'كلية الحاسوب', description: 'مبنى كلية الحاسوب وتقنية المعلومات', isActive: true },
    ...
    { name: 'المكتبة المركزية', description: 'المكتبة المركزية للجامعة', isActive: true },
    { name: 'مبنى الإدارة', description: 'مبنى الإدارة العامة', isActive: true },
  ]);
  ...
  const systems = await systemModel.insertMany([
    ...
    { name: 'حريق', description: 'أنظمة إطفاء الحريق والإنذار', isActive: true },
  ...
  const machines = await machineModel.insertMany([
    // AC Machines
    { name: 'تشيلر', systemId: acSystem!._id, description: 'وحدة تبريد المياه', isActive: true },
    { name: 'AHU', systemId: acSystem!._id, description: 'وحدة معالجة الهواء', isActive: true },
    { name: 'FCU', systemId: acSystem!._id, description: 'وحدة ملف المروحة', isActive: true },
    { name: 'سبليت', systemId: acSystem!._id, description: 'مكيف سبليت', isActive: true },
    { name: 'مكيف شباك', systemId: acSystem!._id, description: 'مكيف شباك', isActive: true },
    // Fire Machines
    { name: 'مضخة حريق', systemId: fireSystem!._id, description: 'مضخة إطفاء الحريق', isActive: true },
    { name: 'لوحة إنذار', systemId: fireSystem!._id, description: 'لوحة الإنذار المركزية', isActive: true },
    { name: 'كاشف دخان', systemId: fireSystem!._id, description: 'كاشف الدخان', isActive: true },
    { name: 'طفاية حريق', systemId: fireSystem!._id, description: 'طفاية الحريق اليدوية', isActive: true },
    // Water Machines
    { name: 'مضخة مياه', systemId: waterSystem!._id, description: 'مضخة رفع المياه', isActive: true },
    { name: 'خزان مياه', systemId: waterSystem!._id, description: 'خزان تخزين المياه', isActive: true },
    { name: 'سخان مركزي', systemId: waterSystem!._id, description: 'سخان المياه المركزي', isActive: true },
    // Electric Machines
    { name: 'مولد كهربائي', systemId: electricSystem!._id, description: 'مولد الطاقة الاحتياطي', isActive: true },
    { name: 'UPS', systemId: electricSystem!._id, description: 'وحدة الطاقة اللامنقطعة', isActive: true },
    { name: 'محول كهربائي', systemId: electricSystem!._id, description: 'محول الجهد الكهربائي', isActive: true },
    { name: 'لوحة توزيع', systemId: electricSystem!._id, description: 'لوحة التوزيع الكهربائية', isActive: true },
    // Elevator Machines
    { name: 'مصعد ركاب', systemId: elevatorSystem!._id, description: 'مصعد نقل الركاب', isActive: true },
    { name: 'مصعد بضائع', systemId: elevatorSystem!._id, description: 'مصعد نقل البضائع', isActive: true },
  ]);
  ...
  const consultant1 = users.find(u => u.email === 'consultant1@maintenance.com');
